    ...
    <div className={cn("rounded-lg border bg-background p-2 shadow-sm", className)} {...props}>
      {label && (
        <div className="grid grid-cols-2 gap-2">
          <div className="font-semibold">{labelFormatter ? labelFormatter(label) : label}</div>
        </div>
      )}
      <div className="grid gap-1">
        {payload.map((item: any, index: number) => (
          <div key={index} className="flex items-center gap-2 text-sm">
            ...
            <div className="font-medium">{formatter ? formatter(item.value, item.name) : item.value}</div>
          </div>
        ))}
      </div>
    </div>
